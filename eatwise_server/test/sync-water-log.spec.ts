...
describe('sync：waterLog op（轻量两态，无 update）', () => {
  ...
  it('create applied：落库并返回服务端视图（含 serverId/version）', async () => {
    const res = await sync.push(userId, [createWaterOp()]);
  ...
  it('create 同键不同体 → IDEMPOTENCY_PAYLOAD_MISMATCH', async () => {
    ...
    const res = await sync.push(userId, [{ ...op, payload: { ...op.payload, amountMl: 500 } }]);
    expect(res.results[0].status).toBe('error');
    expect(res.results[0].error?.code).toBe('IDEMPOTENCY_PAYLOAD_MISMATCH');
    expect(store.waterLogs.size).toBe(1);
  });

  it('create 缺 amountMl/loggedAt → VALIDATION_ERROR', async () => {
  ...
  it('delete：按 serverId 软删；重复删除幂等返回 applied', async () => {
    const created = (await sync.push(userId, [createWaterOp()])).results[0];
  ...
  it('delete：serverId 丢失时按 payload.clientRequestId 兜底定位（响应丢失场景）', async () => {
    ...
    await sync.push(userId, [createWaterOp(rowKey)]);
    const res = await sync.push(userId, [
      {
        clientRequestId: randomUUID(),
        entity: 'waterLog',
  ...
  it('他用户数据隔离：pull 不返回他人 waterLog', async () => {
    const otherId = store.createUser({ phone: '+8613900139000' }).id;
    await sync.push(otherId, [createWaterOp()]);

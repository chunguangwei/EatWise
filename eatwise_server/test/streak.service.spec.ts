...
describe('streak 与补签卡（D-12）', () => {
  ...
  beforeEach(() => {
    store = new DataStore();
    streak = new StreakService(store);
  ...
  it('断签中断后补签可把 streak 连回（达标 2 天 + 断 1 天 + 补签）', () => {
    addQualifiedRecord(addDays(today, -3));
    addQualifiedRecord(addDays(today, -2));
    // 昨天断签 → 补签
    streak.recompute(userId);
    expect(streak.getOrCreate(userId).currentStreak).toBe(0); // 中断归零
    const res = streak.makeUp(userId, randomUUID(), addDays(today, -1)) as {
      currentStreak: number;
    };
    expect(res.currentStreak).toBe(3);
  });

  it('仅可补最近 7 天内：第 8 天前 → 400 MAKEUP_OUT_OF_WINDOW', () => {
    expect(() => streak.makeUp(userId, randomUUID(), addDays(today, -8))).toThrow(
      expect.objectContaining({ code: 'MAKEUP_OUT_OF_WINDOW' }) as unknown as Error,
    );
  });

  it('不能补今天（断食可能仍在进行）→ MAKEUP_OUT_OF_WINDOW', () => {
    expect(() => streak.makeUp(userId, randomUUID(), today)).toThrow(
      expect.objectContaining({ code: 'MAKEUP_OUT_OF_WINDOW' }) as unknown as Error,
    );
  });

  it('重复补同一日期 → 409 MAKEUP_ALREADY_USED', () => {
    const date = addDays(today, -1);
    streak.makeUp(userId, randomUUID(), date);
    expect(() => streak.makeUp(userId, randomUUID(), date)).toThrow(
      expect.objectContaining({ code: 'MAKEUP_ALREADY_USED' }) as unknown as Error,
    );
  });

  it('已达标的日期无需补签 → MAKEUP_ALREADY_USED', () => {
    const date = addDays(today, -1);
  ...
  it('库存用尽（上限 2 张）→ 400 MAKEUP_CARD_EMPTY', () => {
    streak.makeUp(userId, randomUUID(), addDays(today, -1));
    streak.makeUp(userId, randomUUID(), addDays(today, -2));
    expect(() => streak.makeUp(userId, randomUUID(), addDays(today, -3))).toThrow(
      expect.objectContaining({ code: 'MAKEUP_CARD_EMPTY' }) as unknown as Error,
    );
  });

  it('补签幂等：同 clientRequestId 重放返回首次结果，不重复扣卡', () => {
    const date = addDays(today, -1);
    const clientRequestId = randomUUID();
    const first = streak.makeUp(userId, clientRequestId, date);
    const replay = streak.makeUp(userId, clientRequestId, date);
    expect(replay).toEqual(first);
    expect(streak.getOrCreate(userId).makeupCards.stock).toBe(1);
  });

  it('里程碑：currentStreak 跨越 3/7/30 时写入', () => {
    for (let i = 1; i <= 7; i++) addQualifiedRecord(addDays(today, -i));
    const s = streak.recompute(userId);

...
- **走查两连修（2026-09-21, v1.13.3）**：① 数据页专业数据表数值列**去单位**（单位收进表下注 `unitsNote`，i18n 双语）+ `FittedBox(scaleDown)` 双保险，窄屏「2000 千卡」截断成「200…」根治（注意 widget 测试裸数字 finder 不再唯一，断言改带单位大字/计数）。② 「当前按默认目标估算」误提示根治：`nutritionGoalProvider`（mini_signal_cards.dart）不再只信本地快照——快照缺失**或 usedFallback=true**（身体档案表单 sex/活动水平可空，保存即落兜底快照）时按「本地 `loadProfile()` ∪ 服务端下行档案」重算：`profile_sync.dart` 新增 `serverProfileOf`（userView 逆解析）/`mergeProfileWithServer`（本地非空优先 `??` 回落 remote；进食障碍筛查敏感仅本地）/`activityLevelOf`；`userMeProvider.valueOrNull` 同步读缓存（未登录/U1 未加载＝null＝旧兜底行为）；重算仍 `!isComplete` 才保留 usedFallback（有旧兜底快照沿用原值防口径抖动）。纯读侧重算不落盘，权威快照仍由 BodyProfileService 保存档案时写。

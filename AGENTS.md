...
- **手动记运动（2026-09-19 入库）**：无 GMS 设备（鸿蒙等 Health Connect 不可用）手动兜底。MET 系数表 + kcal 估算（MET × 体重kg × 时长h，体重缺省 60kg 兜底并 UI 标注）+ 系统/手动合并纯函数（`mergeBurnKcal` / `mergeSteps` / `estimateKcalFromStepsWalk`：体重 × 距离 × 1.036，距离缺省 步数 × 0.75m 步幅，常量〔待营养背书〕）在 `features/health/domain/exercise_types.dart`；类型表 17 种（走路/慢跑/快跑/骑车/游泳/跳绳/瑜伽/力量/椭圆机/爬山/羽毛球/篮球/足球/乒乓球/网球/健身操/HIIT）。drift 新表 `exercise_logs`（schema v11，设备级纯本地不上行、删除即物理删除；v10 补 `source`：null=手动/'screenshot'=截图导入；v11 补 `steps` 可空——**步数持久化**：走路按步数录入与截图汇总导入落步数快照，数据页「步数」= 系统步数（如有）+ 当日合计），仓储/Provider 在 `features/health/data|application/exercise_log_*`；记录页快捷操作区第五入口「记运动」→ 弹层 `features/health/presentation/exercise_log_sheet.dart`（类型 chips + 时长（走路可改按步数录入，自动换算热量、时长可留空）+ 实时预估 kcal 可覆盖 + 今日列表可删 + D-11 撤销吐司）。首页预算行「运动+Z」与数据页消耗目标环均为 系统活动能量 + 今日手动合计；unsupported 设备无步数时显「—」+ 引导文案「手动记运动可计入消耗」（`TodayBurnCard.stepsGuide`）。**截图导入（同日追加）**：弹层内「拍照识别/相册导入」（复用拍照记选图通道与 ai_engine_guide_card 引导）→ 端侧视觉严格 JSON（prompt/解析/估算纯函数在 `features/health/domain/exercise_screenshot_logic.dart`：活动热量>0 直用，只有步数走 1.036 口径，爬楼仅展示不入账，单次运动消耗优先截图值否则 MET 表，类型映射 17 键+other）→ 可编辑确认弹层 `exercise_screenshot_flow.dart` 确认后落库（source='screenshot'，步数随记录落库）。**展示缺项走查（同日）**：里程碑分享卡「超过了 80% 的伙伴」无真实分位数据支撑属虚假展示，已改自我鼓励口径（streak.milestone.title）。

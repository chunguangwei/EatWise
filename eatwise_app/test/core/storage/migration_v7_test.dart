...
/// schema v6 → v7 迁移测试：v6 库（food_entries 无 during_fast，含历史
/// 饮食记录）打开后 onUpgrade 补 during_fast 列（默认 0 = 非断食期用餐），
/// 历史数据完整保留。
void main() {
  late Directory dir;
  late File dbFile;
  ...
  setUp(() async {
    dir = await Directory.systemTemp.createTemp('eatwise_mig_v7');
  ...
  /// 手工落一个 v6 库（user_version=6，v6 口径 food_entries + 一条记录）。
  Future<void> seedV6Database() async {
    final seed = NativeDatabase(dbFile);
    await seed.ensureOpen(_RawSeedDatabase(seed));
    // v6 口径 food_entries（无 during_fast）。foods 需存在（外键引用）。
    ...
    await seed.runCustom('''
      ...
      VALUES ('l-1', 'anonymous', 'req-1', 'pending',
        '2026-09-17T01:10:00.000Z', '2026-09-17', 'f-rice', 200, 232,
        5.2, 51.8, 0.6, 'manual',
        '2026-09-17T01:10:00.000Z', '2026-09-17T01:10:00.000Z')
    ''');
    await seed.runCustom('PRAGMA user_version = 6');
    await seed.close();
  }

  test('v6 → v7：food_entries 补 duringFast，历史数据保留且默认非断食期用餐', () async {
    ...
    await db.foodEntryDao.updateEntry(
      'l-1',
      const FoodEntriesCompanion(duringFast: Value(true)),
    );
...
/// 裸 executor 的 ensureOpen 宿主（无表，仅用于手工落 v6 schema；
/// 初始打开的建库迁移不影响——随后 PRAGMA 显式置 user_version=6）。

...
/// [pinnedCertDer] 非空时对主 Dio 与 refresh 裸 Dio 启用自签名证书
/// 锁定（仅生产 https://wcg.polin.tech，见 cert_pinning.dart）。
Dio createApiDio({
  ApiConfig? config,
  ...
  if (pinnedCertDer != null) {
    applyCertPinning(dio, pinnedCertDer);
  }
  ...
  if (tokenStore != null) {
    ...
    if (pinnedCertDer != null) {
      applyCertPinning(refreshDio, pinnedCertDer);
    }

...
## 五、修复进度（2026-09-16 @v1.4.1）

| 条目 | 状态 | 说明 |
|---|---|---|
| B1 冷启动空读 | 另行修复（本批不含） | 已在他批处理，勿动 |
| Y1 记录页键盘溢出 38px | ✅ 已修 @v1.4.1 | 空态改可滚动（LayoutBuilder+ConstrainedBox）；键盘顶起时收起「今日已记」汇总行 |
| Y2 AI 表单未交互先亮红 | ✅ 已修 @v1.4.1 | 校验改 touched（失焦）/submitted（首次保存）门控，输入实时重算 |
| Y3 下载续传未生效 | 另行修复（本批不含） | 已在他批处理，勿动 |
| Y4 进度条 0% 看似已满 | ✅ 已修 @v1.4.1 | 轨道改雾灰 20%（withValues 同 home_shell 口径），进度保留品牌绿 |
| Y5 提交后键盘不收起 | ✅ 已修 @v1.4.1 | 记录确认/自定义食物保存/AI 配置保存成功路径统一 unfocus |
| 🔵1 链路卡「——」 | ✅ 已修 @v1.4.1 | 改单破折号「 — 」 |
| 🔵2 开关样式不一致 | ✅ 已修 @v1.4.1 | AppTheme 全局 SwitchThemeData 统一：开=品牌绿+白滑块，关=雾灰浅轨无描边 |
| 🔵3 Custom API 表单无标题 | ✅ 已修 @v1.4.1 | 表单卡补「Custom API / 自定义 API」区块标题（复用 chain.userApi key） |
| 🔵4 搜索框无清空键 | ✅ 已修 @v1.4.1 | 输入非空时显示清空键，新增 i18n `record.search.clear` |
| 🔵5 journey 标签截断+单复数 | ✅ 已修 @v1.4.1 | 成长卡标签允许两行换行；weekly entries 改 slang 复数（1 entry / N entries） |
| 🔵6 进食窗/进食窗口不统一 | ✅ 已修 @v1.4.1 | 统一为「进食窗口」（PRD v1.1 口径，23 处均为「进食窗口」） |
| 🔵7 「供应商」译法 | ✅ 已修 @v1.4.1 | 中文统一改「服务商」（provider/baseUrlRequired/modelRequired 三处） |
| 🔵8 App 名小写+默认图标 | 🟡 部分修复 @v1.4.1 | 显示名改「EatWise」（AndroidManifest label + Info.plist CFBundleDisplayName）；**应用图标遗留**：仍 Flutter 默认图标，需设计稿后替换，下批处理 |
| 🔵9 进食窗态「counts toward」旧口径 | ✅ 已修 @v1.4.1 | 进食态改用将来时新 key `attributionEating`（下一段断食将计入 X / Your next fast will count toward X） |
